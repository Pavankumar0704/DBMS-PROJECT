...
<body style="margin: 0 !important;font-weight: bolder !important;font-family: 'Courier New', Courier, monospace;color: #fff">
    <div style="background-color: #042A38;height: auto;">
        <div class="navbar" style="display: inline-flex;width: 100%;color:#042A38;position:fixed;">
            <section style="margin: 1.5vw;">DEPARTMENT QUIZ MANAGEMENT SYSTEM</section>
		...
		$answers_index_arr = [];
				...
				$q_options_arr = [$row["op1"],$row["op2"],$row["op3"],$row["answer"]];

				shuffle($q_options_arr);
				$answer_index= array_search($row["answer"],$q_options_arr);
				array_push($answers_index_arr,$answer_index);

                    		echo $i.". ".$row["qs"]."<br>";

                    		echo "<input type=\"radio\" value=\"".$j."\" name=\"ans".$i.$j."\">".$q_options_arr[0]."<br>";
                    		echo "<input type=\"radio\" value=\"".($j+1)."\" name=\"ans".$i.$j."\">".$q_options_arr[1]."<br>";               
                    		echo "<input type=\"radio\" value=\"".($j+2)."\"name=\"ans".$i.$j."\">".$q_options_arr[2]."<br>";        
                      		echo "<input type=\"radio\"value=\"".($j+3)."\" name=\"ans".$i.$j."\">".$q_options_arr[3]."<br><br>";
                    		$i++;       
                           
                	}

                	echo "<input id=\"btn\" type=\"submit\" name=\"submit\" value=\"submit\"><br><br><br>";
			foreach($answers_index_arr as $value){
		 		echo "<input type='hidden' name='answers_index_arr[]' value='".$value."' />";
			}
                	echo "</form><br><br>";
            }
            }
            ...
            if(isset($_POST["submit"])){
                ...
                for($i=1;$i<=$count;$i++)
                {
                    if(isset($_POST["ans".$i.$j]) && ($_POST["ans".$i.$j]==$_POST['answers_index_arr'][($i-1)])){
                        $score++;
                    }
